...
In this article I am going to explore a bit of the ZIO streams API and how it can be used to talk to a JMS broker. The [ZIO web site](https://zio.dev/docs/resources/resources) has a lot of very good articles and talks on ZIO streams covering the basics, so I won't repeat those here.
...
Again, we need to create a `JmsSession`, but this time we us it to create a `JmsConsumer` for the given `JmsDestination`.
...
This, we repeat an effect producing optional `Message` objects (optional because the underlying receive yields `None` if no message is available for the consumer.) As we are only interested in the actual messages, we collect only the results actually having a `Message`.
...
For now I have just used Strings as message object; these will be enhanced to more flexible and useful messages.
